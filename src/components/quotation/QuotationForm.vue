...
onBeforeMount(() => {
  ...
  // Auto-fill branch for non-Director users
  if (!isRoleDirector.value && user.value) {
    const userBranch = user.value.branch || (user.value.email && user.value.email.includes('smg') ? 'Semarang' : 'Jakarta')
