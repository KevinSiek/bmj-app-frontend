<template>
  <div class="contain">
    ...
    <div class="lower paginate shadow">
      ...
      <SelectDate />
      <div>
        <div v-if="returnPurchaseOrders?.length == 0 && !isLoading">
          <div class="no-data-text">
            No Data
          </div>
        </div>
        <div v-else class="list">
          <div v-for="(allPO, index) in returnPurchaseOrders" :key="index">
...
watch(() => route.query, (before, after) => {
  ...
  if (JSON.stringify(before) !== JSON.stringify(after)) {
    debounce(() => fetchPurchaseOrder(), 500, 'fetch-purchase-order')
  }
})
