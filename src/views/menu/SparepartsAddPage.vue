            ...
            <template v-for="(branch, index) in sparepart.totalUnit" :key="index">
              <div class="input form-group col-12">
                <div class="branch">
                  ...
                  <input type="number" class="form-control mt-2" :class="{ 'is-invalid': getBranchError(index) }" v-model="branch.stock" placeholder="Stock" @wheel.prevent>
...
const addSparepart = async () => {
  ...
  try {
    ...
    router.push(menuConfig.spareparts.path || menuConfig.spareparts)
  } catch (error) {
